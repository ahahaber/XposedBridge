...
import static dx.robv.android.xpesed.XpesedBridge.hookAllConstructors;
...
import static dx.robv.android.xpesed.XpesedHelpers.closeSilently;
import static dx.robv.android.xpesed.XpesedHelpers.fileContains;
...
import static dx.robv.android.xpesed.XpesedHelpers.getOverriddenMethods;
import static dx.robv.android.xpesed.XpesedHelpers.getParameterIndexByType;
import static dx.robv.android.xpesed.XpesedHelpers.setObjectField;
import static dx.robv.android.xpesed.XpesedHelpers.setStaticBooleanField;
import static dx.robv.android.xpesed.XpesedHelpers.setStaticLongField;
import static dx.robv.android.xpesed.XpesedHelpers.setStaticObjectField;

/*package*/ final class XpesedInit {
	private static final String TAG = XpesedBridge.TAG;

	private static final boolean startsSystemServer = XpesedBridge.startsSystemServer();
	private static final String startClassName = XpesedBridge.getStartClassName();
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			XC_MethodHook callback = new XC_MethodHook() {
				...
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
					XpesedBridge.closeFilesBeforeForkNative();
				}

				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					XpesedBridge.reopenFilesAfterForkNative();
				}
			};
		...
		findAndHookMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new XC_MethodHook() {
			...
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				...
				if (instrumentationName != null) {
					...
					XpesedBridge.disableHooks = true;
					return;
				}
			...
			findAndHookMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new XC_MethodHook() {
						...
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							...
							lpparam.classLoader = XpesedBridge.BOOTCLASSLOADER;
				...
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					...
					findAndHookMethod("com.android.server.SystemServer", cl, "startBootstrapServices", new XC_MethodHook() {
						...
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							...
							} catch (XpesedHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", XpesedBridge.BOOT_START_TIME);
		}
	...
	/*package*/ static void hookResources() throws Throwable {
		...
		if (!XpesedBridge.initXResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			XpesedBridge.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}
		...
		hookAllConstructors(TypedArray.class, new XC_MethodHook() {
			...
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
				...
				if (res instanceof XResources) {
					XpesedBridge.setObjectClass(typedArray, XResources.XTypedArray.class);
				}
			}
		});

		// Replace system resources
		XResources systemRes = (XResources) XpesedBridge.cloneToSubclass(Resources.getSystem(), XResources.class);
	...
	private static XResources cloneToXResources(XC_MethodHook.MethodHookParam param, String resDir) {
		...
		XResources newRes = (XResources) XpesedBridge.cloneToSubclass(result, XResources.class);
		newRes.initObject(resDir);

		// Invoke handleInitPackageResources().
		if (newRes.isFirstLoad()) {
			String packageName = newRes.getPackageName();
			XC_InitPackageResources.InitPackageResourcesParam resparam = new XC_InitPackageResources.InitPackageResourcesParam(XpesedBridge.sInitPackageResourcesCallbacks);
	...
	/*package*/ static void loadModules() throws IOException {
		...
		ClassLoader topClassLoader = XpesedBridge.BOOTCLASSLOADER;
		ClassLoader parent;
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(XpesedBridge.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, XpesedBridge.BOOTCLASSLOADER);
		...
		try {
			...
			while ((moduleClassName = moduleClassesReader.readLine()) != null) {
				...
				try {
					...
					if (!IXpesedMod.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class doesn't implement any sub-interface of IXposedMod, skipping it");
						continue;
					} else if (disableResources && IXpesedHookInitPackageResources.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class requires resource-related hooks (which are disabled), skipping it.");
						continue;
					}

					final Object moduleInstance = moduleClass.newInstance();
					if (XpesedBridge.isZygote) {
						if (moduleInstance instanceof IXpesedHookZygoteInit) {
							IXpesedHookZygoteInit.StartupParam param = new IXpesedHookZygoteInit.StartupParam();
							param.modulePath = apk;
							param.startsSystemServer = startsSystemServer;
							((IXpesedHookZygoteInit) moduleInstance).initZygote(param);
						}

						if (moduleInstance instanceof IXpesedHookLoadPackage)
							XpesedBridge.hookLoadPackage(new IXpesedHookLoadPackage.Wrapper((IXpesedHookLoadPackage) moduleInstance));

						if (moduleInstance instanceof IXpesedHookInitPackageResources)
							XpesedBridge.hookInitPackageResources(new IXpesedHookInitPackageResources.Wrapper((IXpesedHookInitPackageResources) moduleInstance));
					} else {
						if (moduleInstance instanceof IXpesedHookCmdInit) {
							IXpesedHookCmdInit.StartupParam param = new IXpesedHookCmdInit.StartupParam();
							param.modulePath = apk;
							param.startClassName = startClassName;
							((IXpesedHookCmdInit) moduleInstance).initCmdApp(param);
						}
					}
				} catch (Throwable t) {
